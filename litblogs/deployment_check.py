...
def _validate_postgres_ca_custody(
    ...
    *,
    ...
    try:
        ...
        if not stat.S_ISREG(metadata.st_mode) or metadata.st_uid != required_owner_uid:
            raise ValueError
        ...
        if approved_group_ids is None:
            ...
            try:
                ...
                approved_groups.add(grp.getgrnam("litblogs").gr_gid)
            except (ImportError, KeyError):
                pass
            approved_group_ids = frozenset(approved_groups)
        readable_by_runtime = bool(metadata.st_mode & 0o004) or (
            metadata.st_gid in approved_group_ids and bool(metadata.st_mode & 0o040)
        )
        if metadata.st_gid not in approved_group_ids or not readable_by_runtime:
            raise ValueError
