...
def test_postgres_ca_custody_allows_read_only_ca_and_rejects_mutable_ca(tmp_path):
    ...
    postgres_common.validate_postgres_tls_custody(
        ...
        approved_group_ids=frozenset({metadata.st_gid}),
...
def test_backup_publishes_custom_archive_and_matching_checksum_manifest(tmp_path):
    ...
    result = backup_postgres.create_backup(tmp_path, DATABASE_URL, runner=runner)

    assert result.archive.is_file()
    assert result.archive.read_bytes().startswith(b"PGDMP")
    ...
    assert manifest == {
        ...
        "format": "litblogs-postgresql-custom-v1",
        "sha256": hashlib.sha256(result.archive.read_bytes()).hexdigest(),
        "size_bytes": result.archive.stat().st_size,
    }
...
def test_backup_failure_is_redacted_and_never_publishes_partial_output(tmp_path):
    runner = RecordingRunner(
        responses=[
            {
                "returncode": 1,
    ...
    with pytest.raises(postgres_common.PostgresOperatorError) as failure:
        backup_postgres.create_backup(tmp_path, DATABASE_URL, runner=runner)
...
def test_backup_partial_file_is_private_before_pg_dump_writes_student_data(tmp_path):
    ...
    backup_postgres.create_backup(tmp_path, DATABASE_URL, runner=inspect_partial_mode)
...
def test_backup_fsyncs_archive_manifest_and_directory_before_acknowledging(
    ...
    assert file_events[0][1].endswith(".dump.partial")
    assert file_events[1][1].endswith(".manifest.json.partial")
    assert events.index(file_events[1]) < next(
...
def test_backup_requires_an_existing_private_directory(tmp_path):
    ...
    with pytest.raises(postgres_common.PostgresOperatorError, match="directory"):
        backup_postgres.create_backup(missing, DATABASE_URL, runner=RecordingRunner())
...
def test_backup_rejects_relative_output_directory(tmp_path, monkeypatch):
    ...
    with pytest.raises(postgres_common.PostgresOperatorError, match="absolute"):
        backup_postgres.create_backup("backups", DATABASE_URL, runner=RecordingRunner())
...
def test_backup_rejects_noncanonical_or_non_owner_only_output_custody(tmp_path):
    ...
    with pytest.raises(postgres_common.PostgresOperatorError, match="ancestor.*permissions"):
        backup_postgres.create_backup(output, DATABASE_URL, runner=runner)
    assert runner.calls == []
...
def test_backup_entry_point_requires_database_url_without_echoing_it(tmp_path, capsys, monkeypatch):
    ...
    exit_code = backup_postgres.main(["--output-dir", str(tmp_path)])
...
def test_restore_creates_only_a_new_synthetic_database_and_runs_integrity_checks(tmp_path):
    ...
    runner = RecordingRunner(
        responses=[
            ...
            {"returncode": 0, "stdout": "b7c41f0e2d19\n", "stderr": ""},
            {"returncode": 0, "stdout": "ok\n", "stderr": ""},
    ...
    assert result.alembic_revision == "b7c41f0e2d19"
    ...
    assert "LEFT JOIN users" in " ".join(commands[8])
    assert "LEFT JOIN classes" in " ".join(commands[8])
    assert "federated_identities" in " ".join(commands[9])
    assert "BTRIM(identity.issuer)" in " ".join(commands[9])
    assert "BTRIM(identity.subject)" in " ".join(commands[9])
...
def test_verify_existing_is_read_only_and_requires_current_head_and_mapping_count(tmp_path):
    ...
    assert "createdb" not in all_arguments
    assert "pg_restore" not in all_arguments
    assert "drop" not in all_arguments
...
def test_runbook_defines_migration_owner_default_grants_and_runtime_denial_smoke():
    runbook = (
        ROOT_DIR / "docs" / "operations" / "production-runbook.md"
    ).read_text(encoding="utf-8").lower()

    assert "alter default privileges for role litblogs_migrator" in runbook
    assert "grant select, insert, update, delete on tables to litblogs_runtime" in runbook
    assert "grant usage, select on sequences to litblogs_runtime" in runbook
