...
def _production_settings(**overrides):
    values = {
        ...
        "teacher_access_code": "teacher-access-3Z!f7sK2vQ9x",
...
def test_public_runtime_config_is_backend_derived_and_contains_no_secrets(client):
    ...
    for forbidden in ("secret", "password", "database", "vapid_private", "teacher_access"):
        assert forbidden not in serialized


def test_retired_public_admin_codes_are_not_runtime_configuration():
    example = (BACKEND_DIR / ".env.example").read_text(encoding="utf-8")

    assert "admin_access_code" not in Settings.model_fields
    assert "admin_code" not in Settings.model_fields
...
def test_release_attestation_permissions_are_isolated_from_dependency_execution():
    ...
    jobs = workflow["jobs"]
    assert set(jobs) == {"build-release", "attest-release"}
...
def test_release_migrations_use_the_minimal_migration_environment():
    ...
    assert (
        ...
        == migration_step["env"]["TEST_DATABASE_URL"]
    )
...
def test_ci_and_release_run_real_pinned_postgres_backup_restore_smoke():
    ...
    for workflow in (ci, release):
        ...
        assert "POSTGRES_OPERATOR_DATABASE_URL:" in workflow
...
def test_python_dependencies_use_reviewable_inputs_and_hash_locked_installs():
    ...
    assert "pip==26.1.2" in lock_tool_input.read_text(encoding="utf-8")
    assert "pip-tools==7.6.0" in lock_tool_input.read_text(encoding="utf-8")
    ...
    assert '"pip": "26.1.2"' in compiler_text
    assert '"pip-tools": "7.6.0"' in compiler_text
    for workflow in (ci, release):
        assert (
