...
from verify import HOST, PORT, STATE
...
def main():
    ...
    def call(method, path, body=None, *, csrf=False):
        ...
        if csrf:
            headers['X-CSRF-Token'] = cookies['__Host-litblogs-csrf'].value
        try:
    ...
    for key in ('__Host-litblogs-session', '__Host-litblogs-csrf'):
        assert cookies[key]['secure'] and cookies[key]['path'] == '/' and not cookies[key]['domain']
    assert cookies['__Host-litblogs-session']['httponly']
    assert not cookies['__Host-litblogs-csrf']['httponly']
    ...
    assert not cookies['__Host-litblogs-session'].value
    assert call('GET', '/api/auth/session')[0] == 401
