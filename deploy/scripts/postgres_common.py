...
def validate_postgres_tls_custody(
    ...
    *,
    ...
    approved_group_ids: frozenset[int] | None = None,
    trusted_ancestor: str | Path = Path("/"),
) -> None:
    """Require a canonical, immutable CA readable by the intended service group."""
    ...
    if str(resolved) != connection.sslrootcert or not stat.S_ISREG(metadata.st_mode):
        raise PostgresOperatorError("The PostgreSQL root CA custody check failed")
    try:
        _validate_trusted_directory_chain(
            resolved.parent,
    ...
    if metadata.st_uid != required_owner_uid or metadata.st_mode & 0o022:
        raise PostgresOperatorError("The PostgreSQL root CA custody check failed")

    if approved_group_ids is None:
        groups = {0}
        try:
            import grp

            groups.add(grp.getgrnam("litblogs").gr_gid)
        except (ImportError, KeyError):
            pass
        approved_group_ids = frozenset(groups)
    readable_by_intended_identity = bool(metadata.st_mode & 0o004) or (
        metadata.st_gid in approved_group_ids and bool(metadata.st_mode & 0o040)
    )
    if metadata.st_gid not in approved_group_ids or not readable_by_intended_identity:
        raise PostgresOperatorError("The PostgreSQL root CA custody check failed")
